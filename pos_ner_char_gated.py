...
class BiLSTMTagger(object):
    def __init__(self, embed_size, char_hidden_size, word_hidden_size, mlp_layer_size, training_set, dev_set, test_set):
        ...
        self.feedforward_pos = mlp.MLP(self.model, 2, [(word_hidden_size,mlp_layer_size), (mlp_layer_size,len(self.tag_vocab))], 'tanh', 0.0)
        self.feedforward_ner = mlp.MLP(self.model, 2, [(word_hidden_size,mlp_layer_size), (mlp_layer_size,len(self.ner_vocab))], 'tanh', 0.0)

        # Define the gating parameters
        self.w_p = self.model.add_parameters((embed_size, word_hidden_size))
        self.u_p = self.model.add_parameters((word_hidden_size, word_hidden_size))
        self.b_p = self.model.add_parameters((word_hidden_size))
        self.w_n = self.model.add_parameters((embed_size, word_hidden_size))
        self.u_n = self.model.add_parameters((word_hidden_size, word_hidden_size))
        self.b_n = self.model.add_parameters((word_hidden_size))
    ...
    def get_output(self, sents):
        dy.renew_cg()
        w_pos = dy.parameter(self.w_p)
        u_pos = dy.parameter(self.u_p)
        b_pos = dy.parameter(self.b_p)
        w_ner = dy.parameter(self.w_n)
        u_ner = dy.parameter(self.u_n)
        b_ner = dy.parameter(self.b_n)
        tagged_sents = []
        ...
        for sent in sents:
            ...
            for t in range(len(word_reps)):
                ...
                # Predict POS tags
                fwd_val = self.feedforward_pos.forward(h_t_pos)
                pos_probs = dy.softmax(fwd_val).vec_value()
                pred_tag = pos_probs.index(max(pos_probs))

                #Predict NER
                fwd_val = self.feedforward_ner.forward(h_t_ner)
    ...
    def calculate_loss(self, sents):
        ...
        for sent in sents:
            ...
            for t in range(len(word_reps)):
                ...
                # Losses for POS
                pos_prob = self.feedforward_pos.forward(h_t_pos)
                pos_probs_list.append(pos_prob)

                # Compute losses for NER
                ner_prob = self.feedforward_ner.forward(h_t_ner)
